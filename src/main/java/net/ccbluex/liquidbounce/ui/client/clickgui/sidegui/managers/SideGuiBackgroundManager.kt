...
object SideGuiBackgroundManager {
    ...
    fun drawBackgroundCategory(mouseX: Int, mouseY: Int, alpha: Int, drag: Drag, animScroll: Float, rectHeight: Float) {
        ...
        for (mode in backgroundModes) {
            if (yPos + cardHeight <= maxVisibleY) {
                val hovered = RenderHelper.isHovering(xPos, yPos, cardWidth, cardHeight, mouseX, mouseY)
                ...
                RenderPrimitives.drawRect2(xPos.toDouble(), yPos.toDouble(), cardWidth.toDouble(), cardHeight.toDouble(), cardColor)
    ...
    fun drawBackgroundAlphaSlider(mouseX: Int, mouseY: Int, alpha: Int, drag: Drag) {
        ...
        val sliderW = 80f
        val sliderH = 10f
        RenderPrimitives.drawRect2(sliderX.toDouble(), sliderY.toDouble(), sliderW.toDouble(), sliderH.toDouble(), Color(60, 60, 60, alpha).rgb)
        val fraction = (bgAlpha - 1f) / (255f - 1f)
        val fill = sliderW * fraction
        RenderPrimitives.drawRect2(sliderX.toDouble(), sliderY.toDouble(), fill.toDouble(), sliderH.toDouble(), Color(100, 150, 100, alpha).rgb)
        Fonts.InterBold_26.drawString("BG Alpha: ${bgAlpha.toInt()}", sliderX + 2, sliderY - 12, RenderColor.applyOpacity(-1, alpha / 255f))
        val hovered = RenderHelper.isHovering(sliderX, sliderY, sliderW, sliderH, mouseX, mouseY)
        if (hovered && Mouse.isButtonDown(0)) {
    ...
    fun drawBackgroundHexField(alpha: Int, drag: Drag) {
        ...
        RenderPrimitives.drawRect2(quad.x.toDouble(), quad.y.toDouble(), quad.w.toDouble(), quad.h.toDouble(), Color(40, 40, 40, alpha).rgb)
        Fonts.InterBold_26.drawString("Hex:", quad.x, quad.y - 12, RenderColor.applyOpacity(-1, alpha / 255f))
    }
