 ...
 */
public final class IntegerElement extends PanelElement {

    private final ModuleElement element;
    private final IntegerValue setting;
    private boolean dragging;

    public IntegerElement(final ModuleElement element, final Value<?> setting,
                          final Panel parent,
        ...
        this.setting = (IntegerValue) setting;
    }
    ...
    public void drawScreen(final int mouseX, final int mouseY, float partialTicks) {
        ...
        if (dragging) {
            int newValue = (int) Math.round((mouseX - x) * (max - min) / width + min);
