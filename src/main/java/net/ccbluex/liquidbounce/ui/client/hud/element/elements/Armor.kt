...
import net.ccbluex.liquidbounce.utils.render.LnkRenderUtils.Astolfo
...
class Armor(
    ...
    override fun drawElement(partialTicks: Float): Border {
        ...
        if (mc.playerController.isNotCreative) {
            ...
            RUtils.drawRect(-2f, -4f, 2f + x2, 29f, Color(50, 50, 50, 60))

            for (index in 3 downTo 0) {
                val colorall = when {
                    rainbow -> 0
                    colorMode.equals("Astolfo", ignoreCase = true) -> Astolfo(index * speed.get(), saturationValue.get(), brightnessValue.get())
                    else -> color
                }
                val stack = mc.thePlayer.inventory.armorInventory[index] ?: continue
                RUtils.drawGradientSidewaysV(x.toDouble(), 0.0,x.toDouble() + 18 ,17.0,colorall,Color(140,140,140,40).rgb)
                Fonts.font32.drawStringWithShadow(((stack.maxDamage - stack.itemDamage)).toString(),x.toFloat() + 4f,20f,Color(140,140,140,40).rgb)
                RUtils.drawRect(x.toFloat(),25f,x.toFloat() + 18f,26f,Color(140,140,140,220).rgb)
                RUtils.drawRect(x.toFloat(),25f,x.toFloat() + (18f * (stack.maxDamage - stack.itemDamage) / stack.maxDamage),26f,Color(140,140,140,40).rgb)
