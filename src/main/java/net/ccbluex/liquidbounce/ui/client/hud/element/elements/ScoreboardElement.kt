...
class ScoreboardElement(
    ...
    private val shadowStrength = FloatValue("Shadow-Strength", 0F, 0F, 30F, breadWasHere.get())
    private val shadowColorMode = ListValue("Shadow-Color", arrayOf("Background", "Custom"), "Background", breadWasHere.get())

    if (shadowShaderValue.get() && shadowColorMode.get().equals("custom")) {
        val shadowCustom = true
    }

    private val shadowColorRedValue = IntegerValue("Shadow-Red", 0, 0, 255, shadowCustom.get())
    private val shadowColorGreenValue = IntegerValue("Shadow-Green", 111, 0, 255, shadowCustom.get())
    private val shadowColorBlueValue = IntegerValue("Shadow-Blue", 255, 0, 255, shadowCustom.get())
