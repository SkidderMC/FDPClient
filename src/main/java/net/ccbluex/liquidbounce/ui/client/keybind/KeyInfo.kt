...
class KeyInfo(
    ...
    fun render() {
        ...
        RenderUtils.drawRoundedBindRect(0F, 2F, width, height + 8, 6F, shadowColor)
        RenderUtils.drawRoundedBindRect(0F, 0F, width, height, 6F, keyColor)
        Fonts.font40.drawCenteredString(keyName, width * 0.5F, height * 0.9F * 0.5F - (Fonts.font35.FONT_HEIGHT * 0.5F) + 3F, if (hasKeyBind) { usedColor } else { unusedColor }, false)
        GL11.glPopMatrix()
    }
    ...
    fun renderTab() {
        ...
        RenderUtils.drawRoundedBindRect(0F, 0F, baseTabWidth.toFloat(), baseTabHeight.toFloat(), 4F, Color.WHITE.rgb)
        ...
        for (macro in macros) {
            if (yOffset> 0 && (yOffset - 20) <100) {
                ...
                Fonts.fontSmall.drawString(
                    "-", baseTabWidth - 12F - Fonts.font35.getStringWidth("-"), fontHeight, Color.RED.rgb, false
                )
    ...
    fun click(mouseX: Float, mouseY: Float) {
        ...
        if (keyBindMgr.nowDisplayKey == null) {
            ...
            if (scaledMouseY> 22F + Fonts.font40.height &&
                ...
                if (scaledMouseY> baseTabHeight - 22F - Fonts.font35.height) {
                    keyBindMgr.popUI = KeySelectUI(this)
                } else {
                    var yOffset = (12F + Fonts.font35.height + 10F) - stroll
                    for (module in modules) {
