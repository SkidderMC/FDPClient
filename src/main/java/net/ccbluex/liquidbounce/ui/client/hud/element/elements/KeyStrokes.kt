...
class KeyStrokes : Element(5.0, 25.0, 1.5F, Side.default()) {
    ...
    override fun drawElement(partialTicks: Float): Border {
        ...
        if(keyStyleValue.get().equals("Juul")) {
            ...
            FontRenderer.drawString(if (CPSCounter.getCPS(CPSCounter.MouseButton.LEFT).toFloat() != 0f) { CPSCounter.getCPS(CPSCounter.MouseButton.LEFT).toString() + "CPS" } else { "Left" }, 11.5 - (font.getStringWidth(keyName) / 2) + 1, 39.5 - (font.FONT_HEIGHT / 2) + 2, textColor.rgb)
            FontRenderer.drawString(if (CPSCounter.getCPS(CPSCounter.MouseButton.RIGHT).toFloat() != 0f) { CPSCounter.getCPS(CPSCounter.MouseButton.RIGHT).toString() + "CPS" } else { "Right" }, 35.5 - (font.getStringWidth(keyName) / 2) + 1, 39.5 - (font.FONT_HEIGHT / 2) + 2, textColor.rgb)
/* 
