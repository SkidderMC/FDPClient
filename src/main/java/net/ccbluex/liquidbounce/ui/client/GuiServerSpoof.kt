...
class GuiServerSpoof(private val prevGui: GuiScreen) : GuiScreen() {
    ...
    override fun initGui() {
        ...
        textField = GuiTextField(2, Fonts.font20, width / 2 - 100, 60, 200, 20)
    ...
    override fun drawScreen(mouseX: Int, mouseY: Int, partialTicks: Float) {
        drawBackground(0)
        drawCenteredString(Fonts.font20, "%ui.serverSpoof%", width / 2, 34, 0xffffff)
        textField.drawTextBox()
        if (textField.text.isEmpty() && !textField.isFocused)
            drawString(Fonts.font20, "§7%ui.serverSpoof.address%", width / 2 - 100, 66, 0xffffff)
