 ...
 */
package net.ccbluex.liquidbounce.ui.client.clickgui.sidegui.managers
...
object SideGuiConfigsManager {
    ...
    fun drawConfigsCategory(mouseX: Int, mouseY: Int, alpha: Int, drag: Drag, rectWidth: Float) {
        ...
        val isOpenFolderHovered = RenderHelper.isHovering(
        ...
        Fonts.InterBold_26.drawString("OPEN FOLDER", openFolderButtonX + 10, openFolderButtonY + 5, RenderColor.applyOpacity(-1, alpha / 255f))

        val onlineButtonX = xStart
        val onlineButtonY = openFolderButtonY + buttonToggleHeight + buttonSpacing
        val isOnlineHovered = RenderHelper.isHovering(onlineButtonX, onlineButtonY, buttonToggleWidth, buttonToggleHeight, mouseX, mouseY)
        ...
        Fonts.InterBold_26.drawString("ONLINE", onlineButtonX + 10, onlineButtonY + 5, RenderColor.applyOpacity(-1, alpha / 255f))

        val localButtonX = onlineButtonX + buttonToggleWidth + buttonSpacing
        val localButtonY = onlineButtonY
        val isLocalHovered = RenderHelper.isHovering(localButtonX, localButtonY, buttonToggleWidth, buttonToggleHeight, mouseX, mouseY)
        ...
        Fonts.InterBold_26.drawString("LOCAL", localButtonX + 10, localButtonY + 5, RenderColor.applyOpacity(-1, alpha / 255f))
        ...
        if (showLocalConfigs) {
            ...
            if (!localConfigs.isNullOrEmpty()) {
                for (file in localConfigs) {
                    drawSingleConfigButton(mouseX, mouseY, alpha, configX, configY, buttonWidth, buttonHeight) {
                        ...
                        Fonts.InterBold_26.drawString(configName, configX + 5, configY + 5, RenderColor.applyOpacity(-1, alpha / 255f))
                        if (RenderHelper.isHovering(configX, configY, buttonWidth, buttonHeight, mouseX, mouseY) && Mouse.isButtonDown(0)) {
                ...
                Fonts.InterBold_26.drawString("No local configurations available.", configX, configY, RenderColor.applyOpacity(-1, alpha / 255f))
            }
        } else {
            ...
            if (!onlineSettings.isNullOrEmpty()) {
                for (autoSetting in onlineSettings) {
                    drawSingleConfigButton(mouseX, mouseY, alpha, configX, configY, buttonWidth, buttonHeight) {
                        Fonts.InterBold_26.drawString(autoSetting.name, configX + 5, configY + 5, RenderColor.applyOpacity(-1, alpha / 255f))
                        if (RenderHelper.isHovering(configX, configY, buttonWidth, buttonHeight, mouseX, mouseY) && Mouse.isButtonDown(0)) {
                ...
                Fonts.InterBold_26.drawString("No online configurations available.", configX, configY, RenderColor.applyOpacity(-1, alpha / 255f))
            }
        }
    }
    ...
    private inline fun drawSingleConfigButton(
        ...
        val isHovered = RenderHelper.isHovering(configX, configY, width, height, mouseX, mouseY)
        val buttonColor = if (isHovered) Color(70, 70, 70, alpha).rgb else Color(50, 50, 50, alpha).rgb
        RenderPrimitives.drawRect2(configX.toDouble(), configY.toDouble(), width.toDouble(), height.toDouble(), buttonColor)
        drawContent()
    }
