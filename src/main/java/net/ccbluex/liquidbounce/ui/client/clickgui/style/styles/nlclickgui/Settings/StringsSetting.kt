...
class StringsSetting(setting: ListValue, moduleRender: NlModule) : Downward<ListValue>(setting, moduleRender) {
    ...
    override fun draw(mouseX: Int, mouseY: Int) {
        ...
        Fonts.Nl_16.drawString(setting.name, (mainx + 100 + x).toFloat(), (mainy + modey + 57).toFloat(), if (NeverloseGui.getInstance().light) Color(95, 95, 95).rgb else -1)
        RenderUtil.drawRoundedRect((mainx + 170 + x).toFloat(), (mainy + modey + 54).toFloat(), 80f, 14f,
            2F, if (NeverloseGui.getInstance().light) Color(255, 255, 255).rgb else Color(0, 5, 19).rgb,
            1F, Color(13, 24, 35).rgb)
        Fonts.Nl_16.drawString(setting.get(), (mainx + 173 + x).toFloat(), (mainy + modey + 59).toFloat(), if (NeverloseGui.getInstance().light) Color(95, 95, 95).rgb else -1)
        ...
        RenderUtil.drawArrow((mainx + 240 + x).toFloat().toDouble(),
            (mainy + modey + 55 + anim).toFloat().toDouble(), 2, if (NeverloseGui.getInstance().light) Color(95, 95, 95).rgb else Color(200, 200, 200).rgb, length)
        if (setting.openList) {
            GL11.glTranslatef(0f, 0f, 2f)
            RenderUtil.drawRoundedRect((mainx + 170 + x).toFloat(), (mainy + modey + 68).toFloat(), 80f, setting.values.size * 12f,
                2F, if (NeverloseGui.getInstance().light) Color(255, 255, 255).rgb else Color(0, 5, 19).rgb,
                1F, Color(13, 24, 35).rgb)
            for (option in setting.values) {
                val optionIndex = getIndex(option)
                Fonts.Nl_15.drawString(option, (mainx + 173 + x).toFloat(), (mainy + modey + 59 + 12 + optionIndex * 12).toFloat(), if (option.equals(setting.get(), true)) NeverloseGui.neverlosecolor.rgb else if (NeverloseGui.getInstance().light) Color(95, 95, 95).rgb else -1)
            }
            GL11.glTranslatef(0f, 0f, -2f)
        }
    }
