...
class FileSystemAdapter : UltralightFileSystem {
    ...
    private fun log(error: Boolean, fmt: String, vararg args: Any) {
        ...
        if (error) {
            UltralightEngine.logger.error("[ERROR/FileSystem] $message")
        } else {
            UltralightEngine.logger.info("[INFO/FileSystem] $message")
        }
    }
}