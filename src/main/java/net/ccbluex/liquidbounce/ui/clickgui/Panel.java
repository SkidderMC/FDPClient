...
public abstract class Panel extends MinecraftInstance {
    ...
    public void drawScreen(int mouseX, int mouseY, float button) {
        ...
        final int maxElements = Objects.requireNonNull(FDPClient.moduleManager.getModule(ClickGUIModule.class)).getMaxElementsValue().get();
    ...
    private int getElementsHeight() {
        ...
        int count = 0;
        for(final Element element : elements) {
            if (count >= Objects.requireNonNull(FDPClient.moduleManager.getModule(ClickGUIModule.class)).getMaxElementsValue().get())
                continue;
