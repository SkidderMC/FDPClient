...
public class Scaffold extends Module {
    ...
    public void onRender3D(final Render3DEvent event) {
        ...
        for (int i = 0; i < (modeValue.get().equalsIgnoreCase("Expand") ? expandLengthValue.get() + 1 : 2); i++) {
            final BlockPos blockPos = new BlockPos(mc.thePlayer.posX + (mc.thePlayer.getHorizontalFacing().equals(EnumFacing.WEST) ? -i : mc.thePlayer.getHorizontalFacing().equals(EnumFacing.EAST) ? i : 0), mc.thePlayer.posY - (mc.thePlayer.posY == (int) mc.thePlayer.posY + 0.5D ? 0D : 1.0D) - (shouldGoDown ? 1D : 0), mc.thePlayer.posZ + (mc.thePlayer.getHorizontalFacing().equals(EnumFacing.NORTH) ? -i : mc.thePlayer.getHorizontalFacing().equals(EnumFacing.SOUTH) ? i : 0));
            final PlaceInfo placeInfo = PlaceInfo.get(blockPos);

            if (BlockUtils.isReplaceable(blockPos) && placeInfo != null) {
                RenderUtils.drawBlockBox(blockPos, new Color(68, 117, 255, 100), false,true,114514);
                break;
            }
        }
    }
