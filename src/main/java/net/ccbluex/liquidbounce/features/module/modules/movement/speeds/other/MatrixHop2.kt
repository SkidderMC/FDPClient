        ...
        if(mc.thePlayer.motionY<0) {
            ...
            mc.gameSettings.keyBindJump.pressed = false
			      ...
			      if(MovementUtils.getSpeed() < 0.0776)
				        MovementUtils.strafe(0.0776)
			      mc.thePlayer.jump()
			      if(mc.thePlayer.movementInput.moveStrafe == 0) {
				        MovementUtils.INSTANCE.strafe(MovementUtils.getSpeed()*1.0071)
			      }
    ...
    override fun onPacket(event: PacketEvent) {
        ...
        if(packet instanceof S12PacketEntityVelocity) {
			      ...
			      if(Math.sqrt(recX*recX+recZ*recZ) > MovementUtils.getSpeed()) {
				        MovementUtils.strafe(Math.sqrt(recX*recX+recZ*recZ))
