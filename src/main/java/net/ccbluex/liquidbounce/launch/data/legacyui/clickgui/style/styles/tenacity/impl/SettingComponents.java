...
import net.ccbluex.liquidbounce.launch.data.legacyui.clickgui.style.styles.tenacity.utils.animations.impl.EaseInOutQuad;
import net.ccbluex.liquidbounce.launch.data.legacyui.clickgui.style.styles.tenacity.utils.normal.Main;
import net.ccbluex.liquidbounce.launch.data.legacyui.clickgui.style.styles.tenacity.utils.objects.PasswordField;
import net.ccbluex.liquidbounce.launch.data.legacyui.clickgui.style.styles.tenacity.utils.render.DrRenderUtils;
import net.ccbluex.liquidbounce.launch.data.legacyui.clickgui.style.styles.tenacity.utils.render.GuiEvents;
...
public class SettingComponents extends Component {
    ...
    private final HashMap<Value.NumberValue, Float> sliderMap = new HashMap<>();
    private final HashMap<Value.NumberValue, Animation[]> sliderAnimMap = new HashMap<>();
    ...
    public SettingComponents(Module module) {
        ...
        for (Value setting : module.getValues()) {
            ...
            if (setting instanceof Value.NumberValue) {
                sliderMap.put((Value.NumberValue) setting, 0f);
                sliderAnimMap.put((Value.NumberValue) setting, new Animation[]{new DecelerateAnimation(250, 1, Direction.BACKWARDS), new DecelerateAnimation(200, 1, Direction.BACKWARDS)});
            }
    ...
    public void handle(int mouseX, int mouseY, int button, GuiEvents type) {
        ...
        for (Value setting : module.getValues()) {
            ...
            if (setting instanceof Value.NumberValue) {
                Value.NumberValue numberSetting = (Value.NumberValue) setting;
