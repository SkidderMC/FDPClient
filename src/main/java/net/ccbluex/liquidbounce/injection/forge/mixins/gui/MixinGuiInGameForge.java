...
public abstract class MixinGuiInGameForge extends Gui {
    ...
    private void hookRenderHealth(GuiIngameForge instance, int x, int y) {
        if (HUDModule.INSTANCE.handleEvents() && HUDModule.INSTANCE.getBlackHotbar()) {
    ...
    private int hookExperienceLevelYText(FontRenderer instance, String text, int x, int y, int color) {
        return HUDModule.INSTANCE.handleEvents() && HUDModule.INSTANCE.getBlackHotbar() ? instance.drawString(text, x, y - 6, color) : instance.drawString(text, x, y, color);
    }
