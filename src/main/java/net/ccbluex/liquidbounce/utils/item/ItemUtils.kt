...
object ItemUtils {
    ...
    fun hasNBTName(stack: ItemStack): Boolean {
        if (stack.tagCompound != null && stack.tagCompound.hasKey("display", 10)) {
            return stack.tagCompound.getCompoundTag("display").hasKey("Name", 8)
        }
        return false
    }

    fun getWeaponEnchantFactor(stack: ItemStack, nbtedPriority: Float = 0f):Double{
        return (1.25* getEnchantment(stack, Enchantment.sharpness)) +
                (1.0* getEnchantment(stack, Enchantment.fireAspect)) +
                if(hasNBTName(stack)) { nbtedPriority } else { 0f }
    }
    ...
    fun compareArmor(o1: ArmorPiece, o2: ArmorPiece, nbtedPriority: Float = 0f): Int {
        ...
        val compare = RegexUtils.round(getArmorThresholdedDamageReduction(o2.itemStack).toDouble() + if(hasNBTName(o2.itemStack)) { nbtedPriority } else { 0f }, 3).compareTo(
            RegexUtils.round(getArmorThresholdedDamageReduction(o1.itemStack).toDouble() + if(hasNBTName(o1.itemStack)) { nbtedPriority } else { 0f }, 3))

        // If both armor pieces have the exact same damage, compare enchantments
        if (compare == 0) {
            val otherEnchantmentCmp = RegexUtils.round(getArmorEnchantmentThreshold(o1.itemStack).toDouble(), 3).compareTo(
                RegexUtils.round(getArmorEnchantmentThreshold(o2.itemStack).toDouble(), 3))
