...
object ArmorComparator : MinecraftInstance {
    fun getBestArmorSet(stacks: List<ItemStack?>, entityStacksMap: Map<ItemStack, EntityItem>? = null): ArmorSet? {
        val armorMap = getSortedArmorPieces(stacks, entityStacksMap)

        val helmets = armorMap[0]?.ifEmpty { NULL_LIST } ?: NULL_LIST
        val chestplates = armorMap[1]?.ifEmpty { NULL_LIST } ?: NULL_LIST
        val leggings = armorMap[2]?.ifEmpty { NULL_LIST } ?: NULL_LIST
        val boots = armorMap[3]?.ifEmpty { NULL_LIST } ?: NULL_LIST
    ...
    fun getSortedArmorPieces(
        ...
        val droppedStacks = entityStacksMap?.keys.indexedArmorStacks { -1 } ?: emptyList()
        ...
        return (droppedStacks + equippedArmorWhenInChest + inventoryStacks)
            .sortedWith(createComparator(thePlayer, entityStacksMap))
            .groupBy { (it.second.item as ItemArmor).armorType }
    }

    private fun createComparator(
        thePlayer: net.minecraft.entity.player.EntityPlayer,
        entityStacksMap: Map<ItemStack, EntityItem>?,
    ) = Comparator.comparingDouble<Pair<Int?, ItemStack>> { (index, stack) ->
