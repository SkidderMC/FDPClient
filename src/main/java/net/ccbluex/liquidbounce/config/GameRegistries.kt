...
object GameRegistries {
    ...
    val blocks: Map<String, Block> by lazy {
        Block.blockRegistry.keys.mapNotNull { key ->
            Block.blockRegistry.getObject(key)?.let { key.toString() to it }
        }.toMap()
    }

    val items: Map<String, Item> by lazy {
        Item.itemRegistry.keys.mapNotNull { key ->
            Item.itemRegistry.getObject(key)?.let { key.toString() to it }
        }.toMap()
    }

    val effects: Map<String, Potion> by lazy {
        Potion.potionTypes.mapIndexedNotNull { id, potion ->
