# FDPClient Coding Standards
Everyone is invited to participate in the development of FDPClient with pull requests and to open issues on our [issues page](https://github.com/SkidderMC/FDPClient/issues). However, we have to enforce certain standards to keep our code readable, consistent and easier to maintain.

We kindly ask you to use [Kotlin](https://kotlinlang.org/) instead of Java for new code, if possible. In the long term, it is our goal to largely migrate FDPClient to Kotlin.

Contributors: https://github.com/SkidderMC/FDPClient/graphs/contributors
...
If your code is self-contained and not designed exclusively for FDPClient, we may allow you to include it in a separate package outside `net.ccbluex.liquidbounce`. Please note that we have to decide on a case by case basis.
