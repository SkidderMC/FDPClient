...
<script lang="ts">
    ...
    import {gridSize, snappingEnabled} from "../../clickgui/clickgui_store";
    ...
    function findMagneticSnap(center: number, size: number, horizontal: boolean): MagneticSnap | undefined {
        if (isGridIgnored || !$snappingEnabled) {
            return undefined;
        }
    ...
    function snapToGrid(value: number): number {
        ...
        return isGridIgnored || !$snappingEnabled ? value : Math.round(value / size) * size;
    }
    ...
    listen("keyboardKey", (e: KeyboardKeyEvent) => {
        if (e.key === "key.keyboard.left.shift" || e.key === "key.keyboard.right.shift") {
            isGridIgnored = e.action === 1;
        }
    });

    function onBrowserKey(event: KeyboardEvent): void {
        if (event.key === "Shift") isGridIgnored = event.type === "keydown";
    }
...
<svelte:window
        ...
        on:keydown={onBrowserKey}
        on:keyup={onBrowserKey}
        on:blur={onWindowBlur}
/>

<div class="draggable-element" data-component-name={componentName} data-component-id={componentId}
     style="{styleString} z-index: {editorZIndex};" bind:this={element}
    ...
    <div
            ...
            class:selected={inEditor && selected}
...
<style>
    ...
    .editor-mode {
        outline: dashed 1px rgba(255, 255, 255, 0.28);
    ...
    .editor-mode.selected {
        ...
        background-color: rgba(70, 119, 255, 0.08);
    }
