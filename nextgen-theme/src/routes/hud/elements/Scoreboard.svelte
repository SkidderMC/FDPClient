...
{#if processedScoreboard}
    <div class="scoreboard" class:with-shadow={cSettings.shadow} class:without-rect={!cSettings.rect} style="font-family: {cSettings.font ?? 'Inter'}, sans-serif; border-radius: {cSettings.radius ?? 5}px;">
        ...
        <div class="entries">
            {#each processedScoreboard.entries as {name, score}}
                <div class="row">
                    ...
                    {/if}
                    {#if cSettings.number !== false && cSettings.show.includes('Score')}
...
<style lang="scss">
  ...
  .with-shadow { text-shadow: 0 1px 3px #000; }
  .without-rect .entries { background: transparent; }
</style>
