<script lang="ts">
    ...
    import {darken, gridSize, snappingEnabled} from "../../clickgui_store";
...
<div
        class="hud-editor"
        class:grid={dragState !== undefined && $snappingEnabled}
        style="background-size: {toClientCoordinate(Math.max(1, $gridSize))}px {toClientCoordinate(Math.max(1, $gridSize))}px;"
>
    <button class="legacy-switch" type="button" on:click={() => runClientAction("legacy-hud-designer") }>
        Legacy HUD
    </button>
    {#if dragState}
