...
export async function setHudEditorSelected(selected: boolean): Promise<void> {
    const response = await fetch(`${API_BASE}/client/hud-editor`, {
        method: "PUT",
        headers: {
            "Content-Type": "application/json"
        },
    ...
    if (!response.ok) throw new Error("Failed to update the HUD editor state");
}
...
export async function getNativeComponents(): Promise<HudComponent[]> {
    ...
    if (!response.ok) throw new Error("Failed to load native HUD components");
    return await response.json();
}

export async function getComponentCatalog(id: string): Promise<HudComponentCatalogEntry[]> {
    const response = await fetch(`${API_BASE}/client/components/${encodeURIComponent(id)}/catalog`);
    if (!response.ok) throw new Error("Failed to load the HUD component catalog");
    return await response.json();
}

export async function addComponent(id: string): Promise<void> {
    const response = await fetch(`${API_BASE}/client/components/${encodeURIComponent(id)}`, {
        method: "POST"
    });
    if (!response.ok) throw new Error("Failed to add HUD component");
}

export async function setComponentAlignment(id: string, alignment: Alignment): Promise<void> {
    const response = await fetch(`${API_BASE}/client/components/${encodeURIComponent(id)}/alignment`, {
        method: "POST",
        headers: {
            "Content-Type": "application/json"
        },
    ...
    if (!response.ok) throw new Error("Failed to save HUD component alignment");
}

export async function getComponentSettings(id: string): Promise<ConfigurableSetting> {
    const response = await fetch(`${API_BASE}/client/components/${encodeURIComponent(id)}/settings`);
    if (!response.ok) throw new Error("Failed to load HUD component settings");
    return await response.json();
}
...
export async function setComponentSettings(id: string, settings: ConfigurableSetting): Promise<void> {
    ...
    if (!response.ok) throw new Error("Failed to save HUD component settings");
}
